...
func (s service) InsertToShardSeeder(ctx context.Context, param params.ServiceInsertOrdersToShardParam) error {
	err := param.Validate(ctx)
	if err != nil {
		return errors.New(err)
	}
	...
	for _, order := range param {
		...
		orderDBIndex[dbIdx] = append(orderDBIndex[dbIdx], order.ToOrderModelInSeeder())
	}
